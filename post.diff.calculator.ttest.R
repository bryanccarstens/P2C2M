##### Function for calculating differences between the posterior distribution and the posterior predictive distribution using a t-test #####

post.diff.calculator.ttest <- function(summary_stat, post_df, post_pred_df, randomize) { # summary_stat = the statistics being used to compare the distributions, post_df = df of posterior summary stat values with loci as columns, post_pred_df = df of posterior predictive summary stat values with loci as columns
...
### Each locus ###
  ...
  diff_test<-lapply(diff_df, t.test) # perform t test of differences for each locus
...
### Across Loci ###
  ...
  diff_test_all <- t.test(unlist(diff_df)) # perform t test of all differences
  ...
  #write.csv(summary_df_all, paste(summary_stat, "_ttest_1to1_same.csv", sep = ""))
